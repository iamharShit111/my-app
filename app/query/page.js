  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex flex-1">
        <aside className="w-[268px] border-r bg-gray-50 p-4 overflow-y-auto">
          ...
          <ul className="space-y-1">
            {tables.map((table) => (
              <li
                ...
                className={`cursor-pointer px-3 py-2 rounded-md text-sm overflow-hidden ${
                  selectedTable === table
                    ? "bg-violet-600 text-white"
                    : "hover:bg-gray-200 text-gray-800"
                }`}
              >
                <span className="block truncate max-w-full">{table}</span>
              </li>
            ))}
          </ul>
        </aside>
